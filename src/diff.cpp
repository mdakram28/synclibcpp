...
bool get_diff_object(const Json::Value &old_json, const Json::Value &new_json,
    ...
    if (num_deleted == old_json.size()) {
        RET_JSON(new_json);
    } else if (num_replaced == old_json.size()) {
        RET_JSON(new_json);
    }
...
bool get_diff_array(const Json::Value &old_json, const Json::Value &new_json,
    ...
    for (; start < min_length; start++) {
        ...
        if (!get_diff(old_json[start], new_json[start], item_diff, err_msg)) {
            RET_ERROR(err_msg);
        }
...
bool apply_diff_PatchObject(Json::Value &obj, Json::Value &diff, std::string &err_msg) {
    for (Json::Value::const_iterator it = diff.begin(); it != diff.end(); ++it) {
        ...
        if (key == "_t") continue;
        if (obj.isMember(key)) {
            if (get_diff_type(diff[key]) == DiffType::Delete) {
                obj.removeMember(key);
            } else {
                if (!apply_diff(obj[key], diff[key], err_msg)) {
                    RET_ERROR(err_msg);
                }
            }
        } else {
...
bool apply_diff_PatchArray(Json::Value &new_arr, Json::Value &diff, std::string &err_msg) {
    Json::Value old_arr = new_arr;
    for (Json::Value::const_iterator it = diff.begin(); it != diff.end(); ++it) {
        std::string key = it.key().asString();
        if (key == "_t") continue;
        int start = std::stoi(key.substr(0, key.find(":")));
        int end = std::stoi(key.substr(key.find(":") + 1));
        int old_length = old_arr.size();
        ...
        int i = start;
        for (; i < diff_end; i++) {
            if (!apply_diff(new_arr[i], diff[key][i - start], err_msg)) {
                RET_ERROR(err_msg);
            }
        }
